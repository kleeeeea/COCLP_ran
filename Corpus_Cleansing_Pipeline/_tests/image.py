...
def main():
    ...
    with torch.inference_mode():
        ...
        #     group_name="image_pipeline",
        #     repeat_count=1,
        #     delay_seconds=1,
        #     unique_id=11198518358965476818,
        # )
        ...
        mineru_ocr_10 = mineru_ocr.mineru(
            read_path=unzipped_data_homedir +"image_path",
            out_path=unzipped_data_homedir +"image_parsed/ocr",
            backend="pipeline",
            type="image",
        )
        ...
        for q in range(1):
            markdowninfonode_2 = markdowninfonode.execute(
                md_root=get_value_at_index(mineru_ocr_10, 0),
                original_root=unzipped_data_homedir + "image_parsed/jsonl",
                output_file=unzipped_data_homedir + "image_parsed/jsonl/md_info.jsonl",
                threads=1,
            )

            multipruner_3 = multipruner.run_pruners(
                input_dir=get_value_at_index(markdowninfonode_2, 0),
                output_dir=unzipped_data_homedir + "image_parsed/clean",
                num_processes=1,
                min_text_length=100,
            ...
            #     unique_id=18445240148275442164,
            # )

            # batchprocessdocuments_zho_16 = batchprocessdocuments_zho.execute_batch_process(
            #     input_folder=get_value_at_index(multipruner_3, 0),
            #     output_folder="../example/test_out/image/label",
            #     api_key="None",
            ...
            # )
            #
            # batchgenerateqanode_17 = batchgenerateqanode.process(
            #     input_folder=get_value_at_index(batchprocessdocuments_zho_16, 0),
            #     output_folder="../example/test_out/image/QA",
            #     api_key="None",
            #     api_url=get_value_at_index(prompttexteditor_6, 0),
            #     model_name="qwen",
            #     max_workers=1,
            ...
            # )
            #
            # groupexecutorsender_19 = groupexecutorsender.execute(
            #     signal=get_value_at_index(groupexecutorsingle_8, 0),
            #     unique_id=16632935973732032088,
            # )
